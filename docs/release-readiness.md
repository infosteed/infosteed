...
`v0.1.0-beta.4` is InfoSteed's current public Linux amd64 prerelease. InfoSteed is preparing `v0.1.0-beta.6` as its next candidate. Do not treat an untagged checkout or superseded candidate as supported.
...
The resulting evidence will identify the beta.6 candidate commit and immutable image digests. Beta.6 must be tested as an in-place upgrade from beta.1, beta.2, beta.3, beta.4, and beta.5. The beta.1 path must include removal of its temporary Compose and internal-TLS workarounds.
